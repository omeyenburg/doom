...
void I_FinishUpdate (void)
{
	...
	for (int dy = 0; dy < multiply; dy++) {
	    uint32_t *line_ptr = (uint32_t*)image->data + (y * multiply + dy) * (SCREENWIDTH * multiply);

	    for (int x = 0; x < SCREENWIDTH; x++) {
		int idx = screens[0][y * SCREENWIDTH + x];

		#if __BYTE_ORDER == __BIG_ENDIAN
		uint32_t rgb = truecolor_palette[idx][0] | (truecolor_palette[idx][1]<<8) | (truecolor_palette[idx][2]<<16) | (255<<24);
		#else
		uint32_t rgb = truecolor_palette[idx][2] | (truecolor_palette[idx][1]<<8) | (truecolor_palette[idx][0]<<16) | (255<<24);
		#endif

		for (int dx = 0; dx < multiply; dx++) {
		    line_ptr[x * multiply + dx] = rgb;
		}
	    }
	}
    }
